...
import { BookingUI } from "@/lib/types/hike"
...
const ActionsCell = ({ row }: { row: Row<BookingUI> }) => {
  ...
  return (
    <div className="flex items-center justify-center gap-2">
      ...
      <Link
        href={`/hikes/${row.original.hike_id}`}
        className="rounded-md px-4 py-2  hover:bg-gray-100"
...
export const columns: ColumnDef<BookingUI>[] = [
  ...
  {
    accessorKey: "hike_destination",
    header: "Destination"
  },
  {
    accessorKey: "hike_departure_date",
    ...
    <div className="w-full">
      <DataTable
        ...
        data={data.map((d) => ({
          ...
          hike_departure_date: DateTime.fromISO(d.hike.departure_date).toFormat("LLL dd yyyy HH:mm"),
          hike_destination: d.hike.destination,
          paid: d.paid ? "Paid" : "Not Payed",
          user_photo: d.user.profile_picture,
          user_name: `${d.user.first_name} ${d.user.last_name}`
        }))}
        placeholder="Search by destination..."
        searchKey="hike_destination"
        //@ts-ignore
        columns={columns}
        filter={{
          data: [
