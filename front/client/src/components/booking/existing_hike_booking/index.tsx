...
import { deleteBookingById, getHikeBookingById, updateBookHike } from "@/api/booking/hike_booking"
...
interface ExistingHikeBookingProps extends DetailedHTMLProps<HTMLAttributes<HTMLElement>, HTMLElement> {
  id: string
  bookingId: string
}

export default function ExistingHikeBooking({ id, bookingId, className, ...props }: ExistingHikeBookingProps) {
  ...
  const { mutate, isPending } = useMutation<
    ...
    SingleBookingPost[]
  >({
    mutationFn: (d) => updateBookHike(bookingId, d),
    onError: (e) => {
      toast.error(e.response.data.message)
    },
    onSuccess: (res) => {
      toast.success("Booking updated successfully")
      router.push(`/hikes/${id}/booking/${res.data.data._id}/checkout`)
  ...
  const onSubmit: SubmitHandler<BookingSchemaType> = (d) => {
    ...
    if (compareBookingItems(newBooking, oldBooking)) {
      router.push(`/hikes/${id}/booking/${bookingId}/checkout`)
    } else {
      mutate(newBooking)
    }
  }
