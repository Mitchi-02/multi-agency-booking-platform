...
import { createTravel, getFilters } from "@/api/travel"
import { CreateTravelSchemaType } from "@/lib/types/travel"
import { CreateTravelSchema } from "@/lib/schemas/travel"
...
export default function CreateTravelPage() {
  ...
  const {
    ...
    register,
    formState: { errors }
  } = useForm<CreateTravelSchemaType>({
  ...
  const onSubmit: SubmitHandler<CreateTravelSchemaType> = (data) => {
    mutate(data)
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <h1 className="mb-4 text-3xl font-black leading-none tracking-tight">Create a new travel</h1>
      <h2 className="text-xl font-medium leading-none tracking-tight">
        Here you can create a new travel with your destination and services
      </h2>
      <div className="my-6 grid grid-cols-3 justify-between gap-10 rounded-lg bg-[#FCFCFC] p-6">
        ...
        <div>
          ...
          <Select onValueChange={(v) => setValue("region", v)}>
            <SelectTrigger className={`${errors.region && "border-red-500"} w-full`}>
        ...
        <div>
          ...
          <Select onValueChange={(v) => setValue("transportation_type", v)}>
            <SelectTrigger className={`${errors.transportation_type && "border-red-500"} w-full`}>
