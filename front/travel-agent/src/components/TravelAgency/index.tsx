...
import { updateTravelAgency } from "@/api/agency"
...
interface TravelAgencyProps extends DetailedHTMLProps<FormHTMLAttributes<HTMLFormElement>, HTMLFormElement> {
  agency: IAgency
}

export default function TravelAgency({ agency, className, ...rest }: TravelAgencyProps) {
  ...
  const updateTravelAgencyMutation = useMutation({
    mutationFn: (updatedData: UpdateTravelAgencySchemaType) => updateTravelAgency(updatedData),
  ...
  return (
    <form className={cn("", className)} onSubmit={handleSubmit(onSubmit)} {...rest}>
      <h1 className="mb-4 text-3xl font-black leading-none tracking-tight">Travel Agency details</h1>
