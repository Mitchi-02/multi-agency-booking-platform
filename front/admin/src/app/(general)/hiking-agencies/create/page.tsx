...
import { createHikeAgency } from "@/api/hike_agency"
import { Label } from "@/components/ui/label"
import { Textarea } from "@/components/ui/textarea"
import ReadOnlyImage from "@/components/ui/custom/ReadOnlyImage"

export default function CreateHikeAgencyPage() {
  ...
  const createHikeAgencyMutation = useMutation({
    mutationFn: (updatedData: AddHikingAgencySchemaType) => createHikeAgency(updatedData),
  ...
  const onSubmit: SubmitHandler<AddHikingAgencySchemaType> = (data) => {
    createHikeAgencyMutation.mutate(data)
  }

  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <h1 className="mb-4 text-3xl font-black leading-none tracking-tight">Create a new Hike Agency to TripX</h1>
      ...
      <div className="my-6 grid grid-cols-3 justify-between gap-10 rounded-lg bg-[#FCFCFC] p-6">
        ...
        <div className="col-span-3 mt-6 flex items-center justify-end gap-4">
          <Button disabled={createHikeAgencyMutation.isPending} variant="primary" type="submit">
            <Save className="mr-1 h-6 w-6" />
            {createHikeAgencyMutation.isPending ? <Spinner /> : "Create"}
          </Button>
        </div>
      </div>
    </form>
