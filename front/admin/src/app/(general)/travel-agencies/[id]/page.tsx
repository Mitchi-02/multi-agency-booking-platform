...
import { UpdateTravelAgencySchema } from "@/lib/schemas/agency"
import { UpdateTravelAgencySchemaType } from "@/lib/types/agency"
import queryClient from "@/lib/queryClient"
import { deleteTravelAgency, getTravelAgencyDetails, updateTravelAgency } from "@/api/travel_agency"
...
export default function UpdateTravelAgencyPage() {
  ...
  const {
    ...
    formState: { errors },
    watch
  } = useForm<UpdateTravelAgencySchemaType>({
    resolver: zodResolver(UpdateTravelAgencySchema),
    defaultValues: {
      new_photos: [],
      deleted_photos: [],
      logo: ""
  ...
  const { error, data, isFetching } = useQuery<any, CustomAxiosError, TravelAgency>({
    queryKey: ["travelAgencyDetails", id],
    queryFn: () => getTravelAgencyDetails(id)
  })

  const updateTravelAgencyMutation = useMutation({
    mutationFn: (updatedData: UpdateTravelAgencySchemaType) => updateTravelAgency(updatedData, id),
  ...
  const onSubmit: SubmitHandler<UpdateTravelAgencySchemaType> = (data) => {
    updateTravelAgencyMutation.mutate(data)
  }

  const deleteAgencyMutation = useMutation({
    mutationFn: () => deleteTravelAgency(id),
    onSuccess: (data) => {
      toast.success(data.message)
      queryClient.invalidateQueries({
        queryKey: ["travelAgencyDetails", id]
      ...
      <div className="my-6 grid grid-cols-3 justify-between gap-10 rounded-lg bg-[#FCFCFC] p-6">
        ...
        <div className="col-span-3 mt-6 flex items-center justify-end gap-4">
          ...
          <Button disabled={updateTravelAgencyMutation.isPending} variant="primary" type="submit">
            ...
            {updateTravelAgencyMutation.isPending ? <Spinner /> : "Update"}
          </Button>
        </div>
      </div>
    </form>
